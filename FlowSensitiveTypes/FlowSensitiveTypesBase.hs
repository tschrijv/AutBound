module FlowSensitiveTypesBase where
import Data.List 

data Variable = Z | SVarValue Variable | SVarType Variable deriving(Eq)

data Value = Abstraction Term Type | TypeAbstraction Term Type | VTrue | VFalse deriving(Eq)

data Term = TmVariable Variable | TmApply Term Term | TmTypeApply Term Type | TmIf Term Term Term | TmIsEq Term Term | TmAnd Term Term | TmValue Value deriving(Eq)
...
-- Return the Value where onVarValue, onVarType are applied to each
-- Term, Type in the given Value respectively.
...
valuemap :: (Variable -> Term -> Term) -> (Variable -> Type -> Type) -> Variable -> Value -> Value
...
valuemap onVarValue onVarType c (TypeAbstraction typeTerm superType) = (TypeAbstraction (termmap onVarValue onVarType (SVarType c) typeTerm) (typemap onVarType c superType))
valuemap onVarValue onVarType c (VTrue) = (VTrue)
valuemap onVarValue onVarType c (VFalse) = (VFalse)
...
-- argument sub with type Term in the given Value.
valueTermSubstitute :: Term -> Variable -> Value -> Value
valueTermSubstitute sub orig t = (valuemap (termSubstituteHelp sub) (\c x -> x) orig t)

-- Substitute every occurence of the variable orig with the first
-- argument sub with type Type in the given Value.
valueTypeSubstitute :: Type -> Variable -> Value -> Value
...
freeVariablesValue c (TypeAbstraction typeTerm superType) = (nub (concat [(freeVariablesTerm (SVarType c) typeTerm),(freeVariablesType c superType)]))
freeVariablesValue c (VTrue) = (nub (concat [[]]))
freeVariablesValue c (VFalse) = (nub (concat [[]]))
