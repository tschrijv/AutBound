module SystemFSubtypingDeBruijnBase where
import Data.List 

data Variable = Z | SVarValue Variable | SVarType Variable deriving(Eq)

data Value = Abstraction Term Type | TypeAbstraction Term Type deriving(Eq)

data Term = TmVariable Variable | TmApply Term Term | TmTypeApply Term Type | TmValue Value deriving(Eq)

data Type = TypVariable Variable | TypFunction Type Type | TypUniversal Type Type | Top | Typ String deriving(Eq)
