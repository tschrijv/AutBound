...
applyRuleInheritedNamespaces sname rules (iden, rulesOfId) folds lists listSorts table = recurse
  where
    ...
    applyOneRuleLogic sname inst rules (l, RightAdd expr _) folds lists listSorts table param =
      return (ConstrInst ("S" ++ getInstanceNameSpace inst) (stepLogic sname inst rules (l, expr) folds lists listSorts table ++ [param]))
    applyOneRuleLogic sname inst rules (_, RightSub iden context) folds lists listSorts table param
      | (elem iden (map fst lists) || elem iden (map fst folds)) && isJust newrule =
        return (FnCall ("generateHnat" ++ getInstanceNameSpace inst) (FnCall "length" (VarExpr iden : nextStep) : [param]))
      | elem iden (map fst lists) || elem iden (map fst folds) =
        return (FnCall ("generateHnat" ++ getInstanceNameSpace inst) (FnCall "length" [VarExpr iden] : [param]))
      | isJust newrule =
        return (FnCall ("addToEnvironment" ++ fromJust (lookup iden listSorts) ++ context) ((VarExpr iden : nextStep) ++ [param]))
      | otherwise =
        return (FnCall ("addToEnvironment" ++ fromJust (lookup iden listSorts) ++ context) (VarExpr iden : [param]))
      where
    ...
    stepLogic sname inst rules (l, RightAdd expr _) folds lists listSorts table =
      [ConstrInst ("S" ++ getInstanceNameSpace inst) (stepLogic sname inst rules (l, expr) folds lists listSorts table)]
    stepLogic sname inst rules (_, RightSub iden context) folds lists listSorts table
      | (elem iden (map fst lists) || elem iden (map fst folds)) && isJust newrule =
        [FnCall ("generateHnat" ++ getInstanceNameSpace inst) [FnCall "length" (VarExpr iden : nextStep)]]
      | elem iden (map fst lists) || elem iden (map fst folds) =
        [FnCall ("generateHnat" ++ getInstanceNameSpace inst) [FnCall "length" [VarExpr iden]]]
      | isJust newrule =
